...
class OrbitsPDS {
  ...
  async start() {
    ...
    try {
      ...
      // 3) Public List endpoint (GET or POST)
      customApp.post(
        '/xrpc/org.chaoticharmonylabs.orbit.list',
        async (req, res) => {
          console.log('orbit.list handler reached!')
          try {
            // You already build and validate this logic; call into your list handler
            const limit = Number(req.body.limit ?? 50)
            const result = await (this.xrpc as any).methods['org.chaoticharmonylabs.orbit.list']({
              params: { limit },
            })
      ...
      // 4) Public Get endpoint
      customApp.post(
        '/xrpc/org.chaoticharmonylabs.orbit.get',
        async (req, res) => {
          console.log('orbit.get handler reached!')
          try {
            const uri = req.body.uri
            if (!uri) {
              return res.status(400).json({ error: 'URI is required' })
            }
            const result = await (this.xrpc as any).methods['org.chaoticharmonylabs.orbit.get']({
              params: { uri },
            })
            return res.json(result.body)
          } catch (err: any) {
      ...
      // 5) Admin Create endpoint
      customApp.post(
        '/xrpc/org.chaoticharmonylabs.orbit.create',
        async (req, res) => {
          console.log('orbit.create handler reached!')
          try {
            // Check admin header
            requireAdmin(req.headers, this.ADMIN_PASSWORD)
            const input = req.body
            const result = await (this.xrpc as any).methods['org.chaoticharmonylabs.orbit.create']({
              input,
            })
            return res.json(result.body)
          } catch (err: any) {
            console.error(err)
            const status = err.error === 'InvalidRequest' ? 400 : 500
            return res.status(status).json({ error: err.message })
          }
        },
      )

      // 6) Admin Update endpoint
      customApp.post(
        '/xrpc/org.chaoticharmonylabs.orbit.update',
        async (req, res) => {
          console.log('orbit.update handler reached!')
          try {
            requireAdmin(req.headers, this.ADMIN_PASSWORD)
            const input = req.body
            const result = await (this.xrpc as any).methods['org.chaoticharmonylabs.orbit.update']({
              input,
            })
            return res.json(result.body)
          } catch (err: any) {
